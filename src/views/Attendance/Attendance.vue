 ...
 <div>    
  <sweet-modal  ref="modalDelete">
       <FormulateInput
          ...
          label="Delete"
        />
        <CButton type="submit" class="add-submit" :disabled="loadingAdd" @click="deleteDriver">
          Save
        </CButton>
  </sweet-modal>
    <CCardBody class="tab">
      <div class="u" v-if="drivers">
        <CDataTable
          :items="drivers"
          :fields="fields"
          column-filter
        ...
        >
        <td slot="name"  class="it" slot-scope="{ item }">
        <div class="c-avatar">
                  <img v-if="item.profile_img" :src="item.profile_img" class="c-avatar-img" alt="" />
                  <img v-else src="../../../public/img/avatars/7.jpg" class="c-avatar-img" alt="" />
                  <span class="c-avatar-status" :class="`bg-${item.is_online || 'danger'}`"></span>
                </div>
                <div class="it2">
                  <div>{{ item.name }}</div>
                  <div class="small text-muted">
                    <span>
                      <template v-if="item.approved">Approved</template>
                      <template v-else>Wait for approve</template>
                    </span>
                    <br>
                    | Registered At: {{ item.date.substring(0,10) }}
                  </div>
                </div>
                
        </td>
      <template #has_request_on_hold="{item}">
        <td>
            <CIcon
            name="cil-check-alt"
            v-if="item.has_request_on_hold == true"
            class="on"
            height=30
            ></CIcon>
            <CIcon
              name="cil-x"
              v-if="item.has_request_on_hold == false"
              class="off"
              height=30
      ...
      <td>{{item.balance}} SDG</td>
      </template>
      <template #more_info="{ item, index }">
        <td class="py-2">
              <CButton
                color="primary"
                variant="outline"
              ...
              >
                {{ Boolean(item.active) ? "Hide" : "Details" }}
              </CButton>
            </td>
          </template>
          <template #details="{ item }">
            <CCollapse :show="Boolean(item.active)">
              <CCardBody>
                <div>
                  <h4>
                    {{ item.name }}
                  </h4>
                  <p class="text-muted">User since: {{ item.date.substring(0,10) }}</p>
                  <p class="text-muted">ID: {{ item._id }}</p>
                  <p class="text-muted">Phone: {{ item.phone}}</p>
                  <p class="text-muted">Email: {{ item.email }}</p>
                  <p class="text-muted">Active: {{ item.active }}</p>
                  <p class="text-muted">Coordinates: {{ item.coordinates }}</p>
                  <p class="text-muted">Birthday: {{ item.birthday }}</p>
                  <p class="text-muted">Car type: {{ item.car_type }}</p>
                  <p class="text-muted">Own type: {{ item.own_type }}</p>
                  <CButton size="sm" color="info" class="">
                    User Settings
                  </CButton>
                  <CButton size="sm" color="danger" class="ml-1"  @click="showModalDelete(item)">
                    Delete
                  </CButton>
                </div>
              </CCardBody>
...
export default {
  ...
  methods: {
    ...
    deleteDriver(item){
      this.id=item._id
        axios
         .delete("https://alsayar-backend-vwmk7.ondigitalocean.app/api/mobile/driver/delete_driver/"+ this.id,
          {
            headers: { Authorization: `Bearer ${this.token}` },
          }
        )
    ...
    getAllDrivers() {
      axios
        .get(
          "https://alsayar-backend-vwmk7.ondigitalocean.app/api/admin/drivers/get_all_drivers",
          {
            headers: {
              Authorization: `Bearer ${this.token}`,
            },
